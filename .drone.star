...
def main(ctx):
  return [
  linux_cxx("GCC-6", "g++", packages="g++-6", buildtype="boost", buildscript="drone", image=linuxglobalimage, environment={'B2_ARGS': 'cxxstd=98,03,11,14,1y toolset=gcc-6 cxxflags="--coverage -fsanitize=address,leak,undefined -fno-sanitize-recover=undefined -DBOOST_TRAVISCI_BUILD" linkflags="--coverage -lasan -lubsan" "testing.launcher=LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libasan.so.3"', 'GCOVTOOL': 'gcov-6', 'DRONE_JOB_UUID': 'b6589fc6ab'}, globalenv=globalenv),
